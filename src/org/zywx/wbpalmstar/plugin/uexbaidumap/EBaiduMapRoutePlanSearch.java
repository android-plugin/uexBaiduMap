...
public class EBaiduMapRoutePlanSearch implements OnGetRoutePlanResultListener {
	...
	public EBaiduMapRoutePlanSearch(Context context, BaiduMap baiduMap, MapView mapView) {
		mContext = context;
		mBaiduMap = baiduMap;
		mMapView = mapView;
	...
	public void onGetDrivingRouteResult(DrivingRouteResult result) {
		...
		if (result.error == SearchResult.ERRORNO.NO_ERROR) {
			...
			MyDrivingRouteOverlay drivingRouteOverlay = new MyDrivingRouteOverlay(mBaiduMap);
			drivingRouteOverlay.setData(result.getRouteLines().get(0));
			drivingRouteOverlay.addToMap();
			drivingRouteOverlay.zoomToSpan();
			mBaiduMap.setOnMarkerClickListener(drivingRouteOverlay);
			mRoutePlanOverlays.put(mRoutePlanOptions.getId(), drivingRouteOverlay);
		}
	}
	...
	public void onGetTransitRouteResult(TransitRouteResult result) {
		...
		if (result.error == SearchResult.ERRORNO.NO_ERROR) {
			routeNodeIndex = -1;
			mRouteLine = result.getRouteLines().get(0);
			MyTransitRouteOverlay transitRouteOverlay = new MyTransitRouteOverlay(mBaiduMap);
			transitRouteOverlay.setData(result.getRouteLines().get(0));
			transitRouteOverlay.addToMap();
			transitRouteOverlay.zoomToSpan();
			mBaiduMap.setOnMarkerClickListener(transitRouteOverlay);
			mRoutePlanOverlays.put(mRoutePlanOptions.getId(), transitRouteOverlay);
		}
	}
	...
	public void onGetWalkingRouteResult(WalkingRouteResult result) {
		...
		if (result.error == SearchResult.ERRORNO.NO_ERROR) {
			routeNodeIndex = -1;
			mRouteLine = result.getRouteLines().get(0);
			MyWalkingRouteOverlay walkingRouteOverlay = new MyWalkingRouteOverlay(mBaiduMap);
			walkingRouteOverlay.setData(result.getRouteLines().get(0));
			walkingRouteOverlay.addToMap();
			walkingRouteOverlay.zoomToSpan();
			mBaiduMap.setOnMarkerClickListener(walkingRouteOverlay);
			mRoutePlanOverlays.put(mRoutePlanOptions.getId(), walkingRouteOverlay);
		}
	}
	...
	public void nextRouteNode() {
		// 设置节点索引
		if (mRouteLine != null && routeNodeIndex < mRouteLine.getAllStep().size() - 1) {
			routeNodeIndex++;
			showRouteNode();
		} else {
			return;
	...
	private void showRouteNode() {
		if (mRouteLine == null || mRouteLine.getAllStep() == null) {
			return;
		}
		// 获取节结果信息
